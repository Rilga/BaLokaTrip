            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="mb-6 text-right">
                        <a href="{{ route('admin.Products.create') }}" class="px-4 py-2 rounded-md shadow-md text-white" style="padding-right: 20px; background-color: #3717aa;">
                            {{ __('Tambah Produk') }}
                        </a>
                    </div>
                                ...
                                <tbody class="text-sm text-gray-700">
                                    @foreach ($products as $product)
                                        <tr class="border-b hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4">
                                                <img src="{{ asset('storage/images/product/' . basename($product->image)) }}" alt="{{ $product->name }}" class="w-16 h-16 object-cover rounded-md shadow-sm">
                                            </td>
                                            <td class="px-6 py-4 flex justify-start space-x-4">
                                                <!-- Edit Button with Style -->
                                                <a href="{{ route('admin.Products.edit', $product->id) }}" style="background-color: rgb(36, 151, 36)" class="inline-flex items-center px-4 py-2 text-white rounded-md shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
